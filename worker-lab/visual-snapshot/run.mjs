...
for(const [id,url] of routes){
 for(const [suffix,vp] of [["desktop",{width:1280,height:720}],["mobile",{width:390,height:844}]]){
  const c=await browser.newContext({viewport:vp});const p=await c.newPage();const errors=[];p.on("console",m=>{if(m.type()==="error")errors.push(m.text())});p.on("pageerror",e=>errors.push(String(e)));
  ...
  results.push({id,mode:suffix,url,http_status:status,title,ms:Date.now()-t,errors});await c.close();
 }
}
